...
class RND:
    ...
    def reshape_obs(self, obs, env):
        if env.number_diff_server_speeds == 2:
            return np.reshape(obs, (env.number_agents, env.d+1))[:,:2]
        else:
            return np.reshape(obs, (env.number_agents, env.d))

    def get_action(self, env):
        actions = np.zeros([env.number_agents, env.d])
        rnd = np.random.choice(env.d, env.number_agents)
        row_idx = np.arange(len(rnd))
        actions[row_idx, rnd] = 1
        return actions